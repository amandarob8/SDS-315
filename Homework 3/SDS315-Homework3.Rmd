...
#AAPL
model_AAPL = lm(SPY ~ AAPL, data=stocks)
...
#GOOG
model_GOOG = lm(SPY ~ GOOG, data=stocks)
...
#MRK
model_MRK = lm(SPY ~ MRK, data=stocks)
...
#JNJ
model_JNJ = lm(SPY ~ JNJ, data=stocks)
...
#WMT
model_WMT = lm(SPY ~ WMT, data=stocks)
...
#TGT
model_TGT = lm(SPY ~ TGT, data=stocks)
...
COVID-19 deaths in Italy and Spain from February and March of 2020 fit an epxonential growth model. The time variable is the days since first death in the respective country. This data indicates that Italy's growth rate is about .183 or an 18.3% growth rate per day with a doubling time of about 382 days. Similarly, Spain's growth rate is about .276 or a 27.6% growth rate per day with a doubling time of about 253 days.
...
The graph above depicts the COVID-19 deaths by days since first death for Italy and Spain. Spain's growth rate of about 27.6%, greater than Italy's 18.3%, is clear with its steaper curve.
